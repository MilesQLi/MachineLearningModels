...
class hie_rnn(object):
    def __init__(self, n_in, n_h, emb, pre_fix, rnn_cell='lstm', n_h_s=None):
        ...
        self.sentence_lstm = getattr(rnn_cells, rnn_cell)(n_in, n_h, pre_fix+'sen')
        
        
        self.doc_lstm = getattr(rnn_cells, rnn_cell)(n_h, n_h_s, pre_fix+'doc')
        
        self.params = self.sentence_lstm.params+self.doc_lstm.params
        
        result = self.sentence_lstm.get_sentence_output(self.emb[self.x],self.mask)
        hs = result['hs']
        
        result = self.doc_lstm.get_onedoc_output(hs[-1])
        cs_s, hs_s = result['cs'], result['hs']
        self.s_ho = hs_s[-1]
        self.s_co = cs_s[-1]
        ...
        result = self.doc_lstm.get_sentence_output(self.to_sentence, self.s_mask)
        cs_s, hs_s = result['cs'], result['hs']
